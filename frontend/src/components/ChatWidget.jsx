...
const ChatWidget = () => {
    ...
    useEffect(() => {
        ...
        const fetchInitData = async () => {
            try {
                if (user.role === 'owner') {
                    const res = await axios.get(`http://localhost:5000/api/chat/rooms?userId=${user._id}&role=owner`);
                    if (res.data.success) {
                        setRooms(res.data.data);
                    }
                } else {
                    const res = await axios.get(`http://localhost:5000/api/chat/customer-room/${user._id}`);
                    if (res.data.success) {
                        const rId = res.data.data._id;
                        setRoomId(rId);
                        
                        const msgs = await axios.get(`http://localhost:5000/api/chat/messages/${rId}`);
    ...
    const handleSelectRoom = async (room) => {
        ...
        try {
            const msgs = await axios.get(`http://localhost:5000/api/chat/messages/${room._id}`);
            if (msgs.data.success) {
                setMessages(msgs.data.data);
            }
        } catch(err) {
